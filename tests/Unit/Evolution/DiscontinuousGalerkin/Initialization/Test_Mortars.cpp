...
void test_nonconforming_blocks() {
  ...
  const Mesh<3> shell_mesh = domain::Initialization::create_initial_mesh(
      initial_extents, shell, Spectral::Quadrature::GaussLobatto);
  ...
  const TimeStepId next_time_step_id{true, 3, Time{Slab{0.2, 3.4}, {6, 100}}};
  {
    ...
    for (const auto id : shell_neighbor_ids) {
      ...
      const Element<3> neighbor_element =
          domain::Initialization::create_initial_element(id, domain.blocks(),
                                                         initial_refinement);
      const Mesh<3> neighbor_mesh = domain::Initialization::create_initial_mesh(
          initial_extents, neighbor_element,
  ...
  }
  {
    ...
    const Element<3> element = domain::Initialization::create_initial_element(
        element_id, domain.blocks(), initial_refinement);
    const Mesh<3> volume_mesh = domain::Initialization::create_initial_mesh(
        initial_extents, element, Spectral::Quadrature::GaussLobatto);
    DirectionalIdMap<3, Mesh<3>> neighbor_meshes{};
      ...
      for (const auto& neighbor : neighbors) {
        ...
        if (neighbors.are_conforming()) {
          ...
          neighbor_meshes.emplace(
              mortar_id, neighbor_orientation.inverse_map()(
        ...
        // const Mesh<3> neighbor_mesh =
        //     domain::Initialization::create_initial_mesh(
        //         initial_extents, neighbor_element,
        //         Spectral::Quadrature::GaussLobatto);
        //        neighbor_meshes[mortar_id] = neighbor_mesh;
...
void test_p_refine(
  ...
  auto box = db::create<db::AddSimpleTags<
      ...
      mortar_mesh, std::move(mortar_infos), std::move(mortar_next_temporal_id),
...
void test_h_refinement() {
  ...
  tuples::TaggedTuple<
      ...
      domain::Tags::NeighborMesh<2>, ::Tags::TimeStepId, Tags::MortarData<2>,
      Tags::MortarMesh<2>, Tags::MortarInfo<2>, Tags::MortarNextTemporalId<2>,
