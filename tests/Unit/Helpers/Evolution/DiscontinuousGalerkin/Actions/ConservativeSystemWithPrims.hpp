...
template <size_t Dim>
struct ConservativeWithPrimsBoundaryCorrection final
    : public ::evolution::BoundaryCorrection {
  /// \cond
  explicit ConservativeWithPrimsBoundaryCorrection(
      CkMigrateMessage* /*unused*/);
  using PUP::able::register_constructor;
  WRAPPED_PUPable_decl_template(
      ConservativeWithPrimsBoundaryCorrection);  // NOLINT
  /// \endcond

  ConservativeWithPrimsBoundaryCorrection() = default;
  ...
  using dg_package_field_tags = tmpl::list<
      ...
      ::Tags::Flux<Var2<Dim>, tmpl::size_t<Dim>, Frame::Inertial>, Var3>;
...
template <size_t Dim>
PUP::able::PUP_ID ConservativeWithPrimsBoundaryCorrection<Dim>::my_PUP_ID = 0;

template <size_t Dim>
struct ConservativeWithPrimsTimeDerivativeTerms {
  using argument_tags =
...
template <size_t Dim>
struct ConservativeWithPrimsTimeDerivativeTermsWithVariables
    : public ::evolution::PassVariables,
      private ConservativeWithPrimsTimeDerivativeTerms<Dim> {
  using base = ConservativeWithPrimsTimeDerivativeTerms<Dim>;
...
struct ConservativeSystemWithPrims {
  static constexpr size_t volume_dim = Dim;
  static constexpr bool has_primitive_and_conservative_vars = true;
  using boundary_conditions_base =
      domain::BoundaryConditions::BoundaryCondition;
  using boundary_correction = ConservativeWithPrimsBoundaryCorrection<Dim>;
  using compute_volume_time_derivative_terms = tmpl::conditional_t<
      PassVariables, ConservativeWithPrimsTimeDerivativeTermsWithVariables<Dim>,
      ConservativeWithPrimsTimeDerivativeTerms<Dim>>;
