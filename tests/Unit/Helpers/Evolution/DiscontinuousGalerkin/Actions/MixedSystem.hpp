...
template <size_t Dim>
struct MixedBoundaryCorrection final : public ::evolution::BoundaryCorrection {
  /// \cond
  explicit MixedBoundaryCorrection(CkMigrateMessage* /*unused*/);
  using PUP::able::register_constructor;
  WRAPPED_PUPable_decl_template(MixedBoundaryCorrection);  // NOLINT
  /// \endcond

  MixedBoundaryCorrection() = default;
...
template <size_t Dim>
PUP::able::PUP_ID MixedBoundaryCorrection<Dim>::my_PUP_ID = 0;

template <size_t Dim>
struct MixedTimeDerivativeTerms {
...
template <size_t Dim>
struct MixedTimeDerivativeTermsWithVariables
    : public ::evolution::PassVariables,
      private MixedTimeDerivativeTerms<Dim> {
  using base = MixedTimeDerivativeTerms<Dim>;
...
struct MixedSystem {
  ...
  using boundary_correction = MixedBoundaryCorrection<Dim>;
  using compute_volume_time_derivative_terms =
      tmpl::conditional_t<PassVariables,
                          MixedTimeDerivativeTermsWithVariables<Dim>,
                          MixedTimeDerivativeTerms<Dim>>;
