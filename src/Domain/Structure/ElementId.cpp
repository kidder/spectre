...
static_assert(ElementId<1>::max_refinement_level ==
              SegmentId::max_refinement_level);
static_assert(ElementId<2>::max_refinement_level ==
              SegmentId::max_refinement_level);
static_assert(ElementId<3>::max_refinement_level ==
              SegmentId::max_refinement_level);
namespace {
...
size_t get_refinement_level(uint16_t compact_segment_id) {
  ASSERT(compact_segment_id > 0, "Undefined compact segment id");
  return static_cast<uint16_t>(SegmentId::max_refinement_level) -
         static_cast<uint16_t>(std::countl_zero(compact_segment_id));
}

size_t get_index(uint16_t compact_segment_id) {
  ASSERT(compact_segment_id > 0, "Undefined compact segment id");
  return compact_segment_id - std::bit_floor(compact_segment_id);
}
...
ElementId<VolumeDim>::ElementId(uint8_t block_id, uint8_t grid_index,
                                uint8_t direction,
                                uint16_t compact_segment_id_xi,
                                uint16_t compact_segment_id_eta,
...
ElementId<VolumeDim>::ElementId(const size_t block_id, const size_t grid_index)
    : ElementId<VolumeDim>{static_cast<uint8_t>(block_id),
                           static_cast<uint8_t>(grid_index),
                           Direction<VolumeDim>::self().bits(),
                           1,
                           VolumeDim > 1 ? 1 : 0,
                           VolumeDim > 2 ? 1 : 0} {
...
ElementId<1>::ElementId(const size_t block_id,
                        std::array<SegmentId, 1> segment_ids,
                        const size_t grid_index)
    : ElementId<1>{static_cast<uint8_t>(block_id),
                   static_cast<uint8_t>(grid_index),
                   Direction<1>::self().bits(),
                   make_compact_segment_id(segment_ids[0]),
                   uint16_t{0},
                   uint16_t{0}} {
  ASSERT(block_id < two_to_the(block_id_bits),
         "Block id out of bounds: " << block_id << "\nMaximum value is: "
...
ElementId<2>::ElementId(const size_t block_id,
                        std::array<SegmentId, 2> segment_ids,
                        const size_t grid_index)
    : ElementId<2>{static_cast<uint8_t>(block_id),
                   static_cast<uint8_t>(grid_index),
                   Direction<2>::self().bits(),
                   make_compact_segment_id(segment_ids[0]),
                   make_compact_segment_id(segment_ids[1]),
                   uint16_t{0}} {
  ASSERT(block_id < two_to_the(block_id_bits),
         "Block id out of bounds: " << block_id << "\nMaximum value is: "
                                    << two_to_the(block_id_bits) - 1);
...
ElementId<3>::ElementId(const size_t block_id,
                        ...
                        const size_t grid_index)
    : ElementId<3>{static_cast<uint8_t>(block_id),
                   static_cast<uint8_t>(grid_index),
                   Direction<3>::self().bits(),
                   make_compact_segment_id(segment_ids[0]),
                   make_compact_segment_id(segment_ids[1]),
                   make_compact_segment_id(segment_ids[2])} {
  ASSERT(block_id < two_to_the(block_id_bits),
         "Block id out of bounds: " << block_id << "\nMaximum value is: "
                                    << two_to_the(block_id_bits) - 1);
  ASSERT(grid_index < two_to_the(grid_index_bits),
